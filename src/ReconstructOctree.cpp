...
ReconstructOctree::ReconstructOctree(
    int maxNode, Point fullSize, Point endSize, string sil_prefix, int nfile,
    //									 string file_ext, string
    //file_conf,
    string file_ext, int distortion_radius, int rotation_digits,
    float rotationDir, float fxOverDelta, float translation, float skewRoll,
    float skewPitch, float focusOffset, int offsetXleft, int offsetZbottom,
    float offsetImgX, float offsetImgY)
    : Octree(maxNode, fullSize, endSize) {
  cout << "Loading " << nfile << " rotation images with prefix: " << sil_prefix
       << endl;
  ...
  for (int file = 0; file < nfile; file++) {
    ...
    switch (rotation_digits) {
      case 2:
        ...
        if (firstNum == 0 && file < 10)
          filename.append("0");
        else if (firstNum == 1 && file < 9)
          filename.append("0");
        break;
      case 3:
        ...
        if (firstNum == 0 && file < 10)
          ...
          filename.append("00");
        else if (firstNum == 0 && file >= 10 && file < 100)
          filename.append("0");
        else if (firstNum == 1 && file >= 9 && file < 99)
          filename.append("0");
        break;
      default:
...
int ReconstructOctree::getSilStat(int image, int lr, int lc, int hr, int hc) {
  ...
  int ret = iter[hr][hc];
  if (lr > 0 && lc > 0) ret += iter[lr - 1][lc - 1];
  if (lr > 0) ret -= iter[lr - 1][hc];
  if (lc > 0) ret -= iter[hr][lc - 1];
  return ret;
}
...
int ReconstructOctree::getFirstFileNum(string file_prefix, int rotation_digits,
...
unsigned int ReconstructOctree::check(Point lp, Point hp, int& radius,
  ...
  for (int image = 0; image < nimages; image++) {
    perspectiveProject(
        image,
        Point(x[chooselx[image]], y[choosely[image]], z[chooselz[image]]), lr,
        lc);
    perspectiveProject(
        image,
        Point(x[chooserx[image]], y[choosery[image]], z[chooserz[image]]), rr,
        rc);
    perspectiveProject(
        image,
        Point(x[choosetx[image]], y[choosety[image]], z[choosetz[image]]), tr,
        tc);
    perspectiveProject(
        image,
        Point(x[choosebx[image]], y[chooseby[image]], z[choosebz[image]]), br,
        bc);
    ...
    if (isFull && count < (br - tr + 1) * (rc - lc + 1)) isFull = false;
    radius += r;
  }
  // cout << "count " << count << endl;
...
void ReconstructOctree::orthographicProject(int image, Point p, int& row,
                                            int& col) {
  float* iter_rs;
  iter_rs = Rs[image];
  row = (int)floor(scales[0] * (-(p.z - halfSize.z)) + image_center_x);
  col = (int)floor(scales[1] * (iter_rs[0] * (p.x - halfSize.x) +
                                iter_rs[1] * (p.y - halfSize.y)) +
                   image_center_y);
}

void ReconstructOctree::perspectiveProject(int image, Point p, int& row,
                                           int& col) {
  float* iter_rs;
  iter_rs = Rs[image];
  ...
  float transformedTurnX =
      iter_rs[1] * tempX - iter_rs[0] * rotationDir * tempY;
  float transformedTurnY =
      iter_rs[0] * rotationDir * tempX + iter_rs[1] * tempY;
  float transformedTurnZ = tempZ;

  float transformedRollX = transformedTurnX;
  float transformedRollY =
      transformedTurnY * (cosineSkewRoll)-transformedTurnZ * (sineSkewRoll);
  float transformedRollZ =
      transformedTurnY * (sineSkewRoll) + transformedTurnZ * (cosineSkewRoll);

  float transformedPitchX = -transformedRollZ * (sineSkewPitch) +
                            transformedRollX * (cosineSkewPitch);
  float transformedPitchY = transformedRollY;
  float transformedPitchZ =
      transformedRollZ * (cosineSkewPitch) + transformedRollX * (sineSkewPitch);

  tempX = transformedPitchX - focusOffset;
  tempZ = transformedPitchZ;
  tempY = transformedPitchY - translation;

  // to approach orthographic, if fxoverdelta -> infinity then
  // tempFxOverDeltaOverXPlus -> 1
...
void ReconstructOctree::setResolution(Point cubeSize) {
  ...
  int x[2] = {0, cubeSize.x};
  int y[2] = {0, cubeSize.y};
  int z[2] = {0, cubeSize.z};
...
void ReconstructOctree::dropImage() {
  for (int i = 0; i < nimages; ++i) {
    for (int j = 0; j < image_height; ++j) delete[] silhouette_stat[i][j];
    delete[] silhouette_stat[i];
  }
  delete[] silhouette_stat;
}

bool ReconstructOctree::addConsistency(int refImageID, int* consistency,
                                       int* reliability, Point* points,
                                       int npts, double ratio) {
  int i, j;
  int row, col;
  int id;
  int count1, count2;
  ...
  for (i = 0; i < image_height; i++) {
    ...
    for (j = 0; j < image_width; j++) bestMatch[i][j] = -1;
  }
  ...
  for (i = 0; i < npts; i++, p_point++) {
    ...
    orthographicProject(
        refImageID,
        Point(x[chooselx[refImageID]], y[choosely[refImageID]], z[0]), lr, lc);
    orthographicProject(
        refImageID,
        Point(x[choosetx[refImageID]], y[choosety[refImageID]], z[1]), hr, hc);
    // perspectiveProject(refImageID, Point(x[chooselx[refImageID]],
    // y[choosely[refImageID]], z[0]), lr, lc); perspectiveProject(refImageID,
    // Point(x[chooserx[refImageID]], y[choosery[refImageID]], z[1]), hr, hc);
      ...
      for (col = lc; col <= hc; col++)
        if (getSilStat(refImageID, row, col, row, col) > 0) {
          ...
          if (id == -1 || reliability[i] < reliability[id] ||
              reliability[i] == reliability[id] &&
                  consistency[i] > consistency[id]) {
            bestMatch[row][col] = i;
          }
        }
  }
...
void testOctree(ReconstructOctree* mytree, Point cubeSize) {
  ...
  cout << "Total time: " << (endtime - starttime) * 1.0 / CLOCKS_PER_SEC
       << " seconds." << endl;
}
