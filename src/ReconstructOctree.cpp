...
/* ReconstructOctree::ReconstructOctree(int, Point, Point,
                                        std::__cxx11::basic_string<char, std::char_traits<char>,
                                        std::allocator<char> >,
                                        int, std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> >, int, int, float, float, float, float, float, float, int, int, float, float)'
 */
...
void ReconstructOctree::orthographicProject(int image, Point p, int &row, int &col)
{
    ...
    col = (int)floor(scales[1] * (iter_rs[0] * (p.x - halfSize.x) + iter_rs[1] * (p.y - halfSize.y)) + image_center_y);
}
...
bool ReconstructOctree::addConsistency(int refImageID, int *consistency, int *reliability, Point *points, int npts, double ratio)
{
    ...
    for (i = 0; i < image_height; i++)
        for (j = 0; j < image_width; j++)
            if (getSilStat(refImageID, i, j, i, j) > 0)
                count1++;
